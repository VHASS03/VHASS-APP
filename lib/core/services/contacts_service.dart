...
class ContactsService {
  ...
  /// On success, clears local cache so SOS and other flows use the updated list.
  static Future<ApiResponse<void>> deleteContact(String id) async {
    try {
      final response = await ApiService.delete('/contacts/$id');
      if (response.success) {
        await StorageService.clearCachedContacts();
  ...
  /// Ensure contacts are prefetched and refreshed at most once per day.
  /// - If cache is fresh (< 24h), we keep it (instant reads for SOS).
  /// - If cache is missing or stale, we fetch from API in the background.
  static Future<void> preloadContactsIfStale() async {
    try {
      final cachedAt = await StorageService.getContactsCachedAt();
      final now = DateTime.now();

      if (cachedAt != null) {
        final age = now.difference(cachedAt);
        // If cache is less than 24 hours old, no need to hit the API again.
        if (age < const Duration(hours: 24)) {
          print(
            '⏱️ Contacts cache is fresh (${age.inHours}h old) - skipping network fetch',
          );
          return;
        }
      }

      // Either no cache or stale (> 24h) → refresh from server
      print('🔄 Contacts cache stale or missing - refreshing from API...');
      await preloadContacts();
    } catch (e) {
      print('❌ Error in preloadContactsIfStale: $e');
    }
  }
