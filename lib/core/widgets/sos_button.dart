...
class _SOSButtonState extends State<SOSButton>
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      // Start waves as soon as the user touches the button
      onTapDown: (_) {
        setState(() {
          _isPressed = true;
        });
      ...
      // Stop waves when the user lifts the finger
      onTapUp: (_) {
        setState(() {
          _isPressed = false;
        });
      ...
      // Also stop on cancel (finger dragged away, etc.)
      onTapCancel: () {
        setState(() {
          _isPressed = false;
        });
      ...
      // Long press still triggers SOS activation
      onLongPress: widget.onActivate,
      child: RepaintBoundary(
        child: CustomPaint(
          // Show waves ONLY while the button is pressed/held
...
class RipplePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // When not active (button idle), don't draw any waves
    if (!isActive) return;
  ...
  bool shouldRepaint(RipplePainter oldDelegate) {
    // Repaint if animation value changed OR active state changed
