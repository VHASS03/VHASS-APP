...
class VoiceService {
   ...
   * Note: This modifies the fallback list, not the ML model
   */
  addTriggerPhrase(phrase: string): void {
    const normalizedPhrase = phrase.toLowerCase().trim();
    // Since FALLBACK_TRIGGER_PHRASES is readonly, we'd need a mutable array
    // For now, just log the intention
    console.log(`➕ Trigger phrase "${normalizedPhrase}" noted (ML model handles detection)`);
  }

  /**
   * Get current trigger phrases (fallback list)
   */
