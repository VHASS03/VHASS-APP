...
class OllamaService {
   ...
   * Note: This is a simplified version that returns the full response
   * For true streaming, use streamResponseSimple instead
   */
  async getStreamingResponse(
    userId: string,
    userMessage: string
  ): Promise<string> {
    // Use the non-streaming version for simplicity
    return this.getResponse(userId, userMessage);
  }
