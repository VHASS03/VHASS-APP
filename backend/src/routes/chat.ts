...
router.post('/message', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      // Try Groq first, fallback to rule-based chatbot
      try {
        aiResponse = await groqService.getResponse(userId, message);
      } catch (groqError: any) {
        console.warn('Groq unavailable, using fallback:', groqError.message);
        // Use rule-based fallback response
        const fallback = chatbotService.generateBotResponse(message);
