...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // This ensures reliable OTP delivery even if Socket.IO fails
      console.log(`📱 Signup OTP for ${phone}: ${otp}`);
      res.status(201).json({
        success: true,
        message: 'User created. OTP sent via SMS and real-time Socket.IO',
        userId: user._id,
        data: { otp }, // Return OTP for app to send via SMS - CRITICAL FOR SMS DELIVERY
      });
    } catch (error: any) {
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.json({
        ...
        data: { otp }, // Return OTP for app to send via SMS - THIS IS NOT INSECURE IN DEVELOPMENT
      });
    } catch (error: any) {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Activate emergency contacts if they were created during signup
